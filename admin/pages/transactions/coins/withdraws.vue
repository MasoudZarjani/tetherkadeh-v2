...
definePageMeta({
  auth: true,
  breadcrumbs: "لیست برداشت های ارزی",
  pageTransition: {
    name: "rotate",
  },
});
...
const getData = async () => {
  ...
  try {
    const data: any = await api.get(
      `/api/v1/wallet/admin/list?page=${page.value}&limit=${itemsPerPage.value}&search=${search.value}&type=Withdraw&transactionType=Coin`
    );
...
<template>
  <div>
    ...
    <v-card title="لیست برداشت های ارزی" flat>
