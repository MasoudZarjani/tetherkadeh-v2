...
definePageMeta({
  auth: true,
  breadcrumbs: "لیست برداشت های بانکی",
  pageTransition: {
    name: "rotate",
  },
});
...
const headers = ref<ReadonlyHeaders>([
  ...
  { title: "شماره شبا", key: "bankAccount", sortable: false, align: "center" },
  {
    title: "کد رهگیری",
    key: "trackingCode",
    sortable: false,
...
const getData = async () => {
  ...
  try {
    const data: any = await api.get(
      `/api/v1/wallet/admin/list?page=${page.value}&limit=${itemsPerPage.value}&search=${search.value}&type=Withdraw&transactionType=Payment`
    );
  ...
  try {
    ...
    if (status === "Active") {
      payload = {
        ...
        transactionId: trackingCode.value,
...
<template>
  <div>
    ...
    <v-card title="لیست برداشت های بانکی" flat>
      ...
      >
        ...
        <template v-slot:item.status="{ item }: any">
          ...
          <span v-else-if="item.status === 'Inactive'" class="text-error">
            غیرفعال
          </span>
