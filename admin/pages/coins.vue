...
import type { Symbol } from "~/types/symbol.types";
...
const headers: VDataTable["$props"]["headers"] = [
  ...
  { title: "نام ارز", key: "name", sortable: false, align: "start" },
...
const defaultSymbol: Symbol = {
  ...
  buyingPriceGap: 0,
  sellingPriceGap: 0,
  buyingPriceGapPercentage: 0,
  sellingPriceGapPercentage: 0,
  numberOfDecimalPlaces: 0,
  status: false,
};

const symbolItem = ref<Symbol>({ ...defaultSymbol });
...
const getData = async () => {
  try {
    ...
    const { data, total }: any = await api.get(
      `/api/v1/symbol/admin?page=${page.value}&limit=${itemsPerPage.value}&search=${search.value}`
    );
    items.value = data;
    totalItems.value = total;
  } catch (error) {
    showAlert({ text: "خطا در دریافت اطلاعات", color: "error" });
  } finally {
    toggleOverlay(false);
...
const symbolKeys: (keyof Symbol)[] = [
  ...
  "buyingPriceGap",
  "sellingPriceGap",
  "buyingPriceGapPercentage",
  "sellingPriceGapPercentage",
...
const edit = async () => {
  try {
    toggleOverlay(true);
    isLoading.value = true;
    const cleanedSymbol = filterByKeys<Symbol>(symbolItem.value, symbolKeys);
    await api.patch(`/api/v1/symbol/${selectedId.value}`, cleanedSymbol);
    showAlert({ text: "اطلاعات با موفقیت ویرایش شد.", color: "success" });
    getData();
    editDialog.value = false;
  } catch {
    showAlert({ text: "خطا در ذخیره اطلاعات", color: "error" });
  } finally {
    isLoading.value = false;
    toggleOverlay(false);
  }
...
<template>
  <div>
    ...
    >
      <v-card>
        <v-toolbar>
          ...
          <v-toolbar-title>ویرایش ارز صرافی</v-toolbar-title>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="12" sm="3">
              <v-text-field
                v-model="symbolItem.numberOfDecimalPlaces"
                type="text"
                label="حداکثر اعشار"
                variant="outlined"
                class="my-2"
            ...
            <v-col cols="12" sm="3">
              <v-text-field
                v-model="symbolItem.buyingPriceGap"
                type="number"
                label="گپ قیمت خرید"
                variant="outlined"
                class="my-2"
            ...
            <v-col cols="12" sm="3">
              <v-text-field
                v-model="symbolItem.sellingPriceGap"
                type="number"
                label="گپ قیمت فروش"
                variant="outlined"
                class="my-2"
            ...
            <v-col cols="12" sm="3">
              <v-text-field
                v-model="symbolItem.buyingPriceGapPercentage"
                type="number"
                label="درصد گپ قیمت خرید"
                variant="outlined"
                class="my-2"
              ></v-text-field>
            </v-col>
            <v-col cols="12" sm="3">
              <v-text-field
                v-model="symbolItem.sellingPriceGapPercentage"
                type="number"
                label="درصد گپ قیمت فروش"
                variant="outlined"
                class="my-2"
              ></v-text-field>
            </v-col>
    ...
    <v-card title="تنظیمات ارزها" flat>
