  ...
  <div class="TextInput" :class="{ 'has-error': !!error }">
    ...
    >
      ...
      <div class="group relative">
        ...
        <div
          class="absolute bg-white text-black p-4 rounded-lg hidden group-hover:block bottom-6 right-0 w-96 text-xs"
        >
          {{ $t('mobileHint') }}
        </div>
      </div>
    ...
    />
    <p class="help-message text-red-500 mt-1 text-xs font-12"   :class="locale === 'fa' ? 'text-right' : 'text-left'" v-if="error">
      {{ $t(error) }}
    </p>
  </div>
</template>
