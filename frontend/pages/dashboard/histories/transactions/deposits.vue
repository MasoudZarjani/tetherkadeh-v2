...
useSeoMeta({
  title: `صرافی ${site.value?.siteName} | تاریخچه واریزی ها`,
  ogTitle: `صرافی ${site.value?.siteName} | تاریخچه واریزی ها`,
...
const getData = async () => {
  try {
    ...
    const data: any = await api.get(
      `/api/v1/wallet/transactions?page=${page.value}&limit=${limit.value}&sort=createdAt&order=-1&type=Deposit&search=${search.value}`
    )
  ...
  <div class="container mx-auto px-4 lg:px-8 py-2 lg:py-4">
    <CoreCardDefault class="lg:col-span-2 p-2 lg:p-6">
      <div class="flex flex-col lg:flex-row justify-between items-center mb-4 w-full">
        <CoreInputTabHistoryTransactions currentTab="deposits" />
